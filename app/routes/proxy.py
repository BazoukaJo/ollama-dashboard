...
Native Ollama clients use ``/ollama/api/...``; VS Code Copilot and other OpenAI-
compatible clients use ``/ollama/v1/chat/completions`` (and ``/ollama/v1/models``).
...
def _load_dashboard_options(model_name):
    ...
    if not settings_path.exists() or not model_name:
...
def _forward_v1_via_native_api(native_payload, native_endpoint, openai_payload):
    ...
    if stream:
        upstream = requests.post(upstream_url, json=native_payload, stream=True)
    ...
    response_data = requests.post(upstream_url, json=native_payload)
...
def proxy_ollama_root():
    ...
    return jsonify({
        'success': True,
        'message': (
            'Ollama dashboard proxy — Copilot: /ollama/v1/chat/completions; '
            'models: /ollama/api/tags or /ollama/v1/models'
        ),
