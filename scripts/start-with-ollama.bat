...
if %ERRORLEVEL% EQU 0 (
    ...
    tasklist /FI "IMAGENAME eq python.exe" 2>NUL | find /I "OllamaDashboard.py">NUL
    if %ERRORLEVEL% EQU 0 (
        ...
        cd /d "%~dp0.."
        start "Ollama Dashboard" python OllamaDashboard.py
    ...
    if %ERRORLEVEL% EQU 0 (
        ...
        for /f "tokens=2" %%i in ('tasklist /FI "IMAGENAME eq python.exe" ^| find /I "OllamaDashboard.py"') do (
